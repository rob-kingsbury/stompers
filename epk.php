<?php
$page_title = 'EPK | Swamp City Stompers';
$page_description = 'Electronic Press Kit for the Swamp City Stompers - photos, bio, tech rider, and booking info.';
?>
<!DOCTYPE html>
<html lang="en" class="no-js">
<head>
<?php include 'includes/head.php'; ?>
</head>
<body data-barba="wrapper">
<?php include 'includes/nav.php'; ?>

  <main class="page-main" id="main-content" data-barba="container" data-barba-namespace="epk">
    <section class="page-hero page-hero--epk">
      <div class="page-hero-content">
        <span class="section-number">PRESS KIT</span>
        <h1 class="page-title">EPK</h1>
      </div>
    </section>

    <section class="content-section">
      <div class="epk-grid">
        <div class="epk-card">
          <h2>Band Bio</h2>
          <p>Short and long-form bios for press and promotional use.</p>
          <a href="#" class="btn btn-outline">Download PDF</a>
        </div>

        <div class="epk-card">
          <h2>Press Photos</h2>
          <p>High-resolution promotional photos, print and web ready.</p>
          <a href="#" class="btn btn-outline">Download ZIP</a>
        </div>

        <div class="epk-card">
          <h2>Tech Rider</h2>
          <p>Stage plot, input list, and technical requirements.</p>
          <a href="#" class="btn btn-outline">Download PDF</a>
        </div>

        <div class="epk-card">
          <h2>Logo Pack</h2>
          <p>Vector logos in various formats for print and digital.</p>
          <a href="#" class="btn btn-outline">Download ZIP</a>
        </div>
      </div>
    </section>

    <section class="content-section epk-stats">
      <h2 class="section-title">At a Glance</h2>
      <div class="stats-grid">
        <div class="stat-item">
          <span class="stat-value">4</span>
          <span class="stat-label">Band Members</span>
        </div>
        <div class="stat-item">
          <span class="stat-value">2019</span>
          <span class="stat-label">Formed</span>
        </div>
        <div class="stat-item">
          <span class="stat-value">200+</span>
          <span class="stat-label">Shows Played</span>
        </div>
        <div class="stat-item">
          <span class="stat-value">Ottawa</span>
          <span class="stat-label">Home Base</span>
        </div>
      </div>
    </section>

    <section class="content-section">
      <div class="epk-info">
        <div class="info-block">
          <h3>Genre</h3>
          <p>Swamp Blues / Southern Rock / Roots Americana</p>
        </div>
        <div class="info-block">
          <h3>For Fans Of</h3>
          <p>ZZ Top, Creedence Clearwater Revival, The Black Keys, Rival Sons, Black Crowes</p>
        </div>
        <div class="info-block">
          <h3>Set Length</h3>
          <p>Available for 45-minute, 90-minute, or full evening sets</p>
        </div>
      </div>
    </section>

    <section class="cta-section">
      <div class="cta-content">
        <h2>Book Us</h2>
        <p>For booking inquiries, please get in touch.</p>
        <a href="contact.php" class="btn btn-primary">Contact</a>
      </div>
    </section>
  </main>

</body>
</html>
